...
// TCD1304DG output is inverted: dark≈high voltage, bright≈low voltage.
// We emit (1023-v) so 0=dark, 1023=bright — correct for graph and tracker.
#define CCD_STRIDE 8
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // CCD_RAW — read A0 directly (no CCD clocking) to verify pin connectivity
    if (input == F("CCD_RAW")) {
      uint16_t raw = (uint16_t)analogRead(ccdOsPin);
      Serial.print(F("RAW_A0="));
      Serial.println(raw);
      return;
    }
